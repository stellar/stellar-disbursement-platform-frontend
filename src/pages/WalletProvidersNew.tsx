...
export const WalletProvidersNew = () => {
  ...
  const handleClickOutside = useCallback((event: MouseEvent) => {
    ...
    const inputElement = assetsInputRef.current?.querySelector("input");
  ...
  const handleAddNewWallet = (e: any) => {
    e.preventDefault();
  ...
  // TODO: success modal and error message for response
  ...
  const validateField = (fieldId: FormField, value: string) => {
    ...
    switch (fieldId) {
      ...
      case "homepage":
        if (!value) {
          ...
          errorMsg = isValidUrl(value) ? "" : "Invalid URL";
        }
        break;
      case "sep_10_client_domain":
        if (!value) {
          ...
          errorMsg = isValidDomain(value) ? "" : "Invalid domain";
        }
        break;
      case "deep_link_schema":
        if (!value) {
          ...
          errorMsg = isValidDeepLink(value) ? "" : "Invalid deep link schema";
        }
        break;
      default:
      // do nothing
    ...
    if (errorMsg) {
      setFormFieldErrors({ ...formFieldErrors, [fieldId]: errorMsg });
    } else {
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const { [fieldId]: _, ...rest } = formFieldErrors;
      setFormFieldErrors(rest);
    }
  };
  ...
  return (
    <>
      ...
      <form onSubmit={handleAddNewWallet}>
        <Box gap="lg" addlClassName="WalletProvidersNew">
          ...
          <Box gap="lg" direction="row" justify="end">
            <Button size="md" variant="primary" disabled={!isSubmitEnabled()}>
              Add a new wallet
            </Button>
          </Box>
        </Box>
