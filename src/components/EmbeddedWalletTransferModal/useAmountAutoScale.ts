...
const getChevronWidth = (assetButton: HTMLButtonElement | null, fallback: number) =>
  assetButton?.querySelector("svg")?.getBoundingClientRect().width ?? fallback;

const getAvailableTextWidth = (
  rowWidth: number,
  rowStyles: CSSStyleDeclaration,
  assetButton: HTMLButtonElement | null,
  chevronFallback: number,
) => {
  const rowGap = readGap(rowStyles);
  const assetStyles = assetButton ? window.getComputedStyle(assetButton) : null;
  const assetGap = assetStyles ? readGap(assetStyles) : 0;
  const assetMarginLeft = assetStyles ? readMarginLeft(assetStyles) : 0;
  const chevronWidth = getChevronWidth(assetButton, chevronFallback);

  return rowWidth - rowGap - assetGap - assetMarginLeft - chevronWidth;
};
...
export const useAmountAutoScale = ({
  ...
  const updateAmountFontSize = useCallback(() => {
    ...
    const chevronFallback = readCssNumber(rowStyles, "--ew-amount-chevron-width") ?? 0;
